...
def nondimensional(diff: float, activity: float, debug_plots: bool = False):
    ...
    Pab_eq_7['g'] = 0.5 * (1 + Pa['g'] * np.exp(-(Eb_Pab.v - Eb_Pa.v)) - np.sqrt(
        (1 + Pa['g'] * np.exp(-(Eb_Pab.v - Eb_Pa.v))) ** 2 - 4 * Pa['g'] * np.exp(-(Eb_Pab.v - Eb_Pa.v)) * (
                    f_B['g'] - Pb['g'])))
    Pab_eq_8['g'] = 0.5 * (1 + Pb['g'] * np.exp(-(Eb_Pab.v - Eb_Pb.v)) - np.sqrt(
        (1 + Pb['g'] * np.exp(-(Eb_Pab.v - Eb_Pb.v))) ** 2 - 4 * Pb['g'] * np.exp(-(Eb_Pab.v - Eb_Pb.v)) * (
                    f_A['g'] - Pa['g'])))
    ...
    problem.add_equation(
        "Pab_eq_7 = 0.5*(1+Pa*np.exp(-(Eb_Pab.v-Eb_Pa.v))-np.sqrt( (1+Pa*np.exp(-(Eb_Pab.v-Eb_Pa.v)))**2 -4*Pa*np.exp(-(Eb_Pab.v-Eb_Pa.v))*(f_B-Pb)))")
    problem.add_equation(
        "Pab_eq_8 = 0.5*(1+Pb*np.exp(-(Eb_Pab.v-Eb_Pb.v))-np.sqrt( (1+Pb*np.exp(-(Eb_Pab.v-Eb_Pb.v)))**2 -4*Pb*np.exp(-(Eb_Pab.v-Eb_Pb.v))*(f_A-Pa)))")
